...
var InvalidStateActionError = BaseError.extend(function(self, message) {
    self.name = "InvalidStateActionError";
    self.message = "Invalid state action: " + message;
});
    ...
    self.add.sessions_until_state = function(state, label) {
        ...
        */
        ...
        self.im.on('session:new', function(e) {
            ...
            if (typeof metadata[metadata_label] === 'undefined'){
                metadata[metadata_label] = 0;
            }

            e.im.user.metadata[metadata_label] += 1;
        });

        self.im.on('state:' + action, function(e) {
            /* Reset sessions counter and fire metric */
            if(e.state.name === state.state) {
                metadata_value = e.state.im.user.metadata[metadata_label];
                e.state.im.user.metadata[metadata_label] = 0;
