    ...
    self.add.tracker = function(start_trigger, end_trigger, metrics) {
        /** MetricsHelper.add.tracker(start_trigger, end_trigger, metrics)
            ...
            :param object start_trigger:
                The action that should trigger the start of the metric. Of
                the format ``{state_action: state_name}``, e.g.
            ...
            :param object end_trigger:
                The action that should trigger the end of the metric. Of
                the format ``{state_action: state_name}``, e.g.
        ...
        */
        var action_start = Object.keys(start_trigger)[0];
        var action_end = Object.keys(end_trigger)[0];
        var state_from = {
            state: start_trigger[action_start],
            action: action_start
        };
        var state_to = {
            state: end_trigger[action_end],
            action: action_end
        };

        if(metrics.time_between_states) {
            self.add.time_between_states(
                state_from, state_to, metrics.time_between_states);
        }
    ...
    self.add.trigger = function(trigger, metrics) {
        /** MetricsHelper.add.trigger(trigger, metrics)
            ...
            :param object trigger:
                The action that should trigger the metrics. Of the format 
                ``{state_action: state_name}``, e.g.
        ...
        */
        var action = Object.keys(trigger)[0];
        var state = { state: trigger[action], action: action };

        if(metrics.sessions_until_state) {
            self.add.sessions_until_state(state, metrics.sessions_until_state);
        }
        if(metrics.total_state_actions) {
            self.add.total_state_actions(state, metrics.total_state_actions);
        }
